...
app.post("/login", (req, res) => {
    ...
    if (emailExists) {
        ...
        if (password === correctPassword) {
            res.send("You have successfully logged in as " + email + "!")
        }else{
            res.redirect(`/error?code=${401}&message="Incorrect Password"`)
        }
    }else{
        res.redirect(`/error?code=${401}&message="This Email is not registered"`)
    }
