...
app.post("/login", (req, res) => {
    ...
    if (emailExists) {
        ...
        if (password === correctPassword) {
            ...
            let code = db.get("tokens").find({[`"${email}"`]: any}).value() //find anything that has the key "{emailoftheguy here}"

            console.log(code)

            if (!code) {
                code = Math.round(Math.random() * 100000000)
                db.get("tokens").push({[email]: code}).write()
            }



            res.render('home', { email, points, code })
        }else{
