...
async function verifyUser(username, email, password) {
    ...
    try {
        const result = await collection.findOne({ username: username, email: email}); //this works 
        //console.log(result);  // THIS SHOULD NOT HAVE A TEXT AND + BCUZ IT THEN CONSOLE.LOGS OBJECT OBJECT
        ...
        if (result != null) {
            ...
            return {ok: false, user: null, error: "User not found :("}
        }
